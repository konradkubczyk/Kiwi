...
class Schedule {
    ...
    async #getData(scheduleURL) {
        ...
        try {
            ...
            return await parser.parseStringPromise(scheduleXML);
        } catch (error) {
    ...
    async getICS() {
        ...
        try {
            ...
            for (const event of events) {
                const summary = this.properties.compact
                    ? event['przedmiot'][0]
                    : this.properties.reverse
                        ? (event['przedmiot'][0] + ' | ' + event['typ'][0].charAt(0).toUpperCase() + event['typ'][0].slice(1))
                        : (event['typ'][0].charAt(0).toUpperCase() + event['typ'][0].slice(1) + ' | ' + event['przedmiot'][0]);
